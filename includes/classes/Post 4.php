<?php
		class Post{
			...
			if($check_empty !=""){
				...
				$body = implode(" ", $body_array); //End of Video script
				...
				// If user is on own profile, user_ to is 'none'
				if($user_to == $added_by){
					$user_to ="none";
				}
             ...
             $no_punctuation = preg_replace("/[^a-zA-Z 0-9]+/", "", $body); // Anything that is not letter or number, replcase it with blank
	...
	if(mysqli_num_rows($data_query) > 0) {
		...
		while($row = mysqli_fetch_array($data_query)){
			...
			if($row['user_to'] == "none"){
				$user_to ="";
					...
					$user_to = " to <a href='" . $row['user_to'] ."'>" . $user_to_name . "</a>"; // Returns  The first and Last name of the UserLoggedIn  
				}
				...
				// Iterating through all the posts already loaded
			...
			// Returns user firstname, lastname and profile_pic
		...
		public function loadProfilePosts($data, $limit){
			...
			$str .=	"<div class='status_post' onClick='javascript:toggle$id()'><br><br>
							<div class='post_profile_pic'>
								<img src='$profile_pic' width ='50'>
							</div>
	 ...
	 #All the code sections from Method->loadPostsFriend
	$userLoggedIn = $this->user_obj->getUsername();
...
<?php
		...
		echo "<p>No post found. If you cliked a link, connection may have been broken or out of bound!</p>";
		return; 
	}
		echo $str;
